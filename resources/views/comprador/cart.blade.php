<x-app-layout>
    <div class="container mx-auto mt-12 p-8 bg-white rounded-lg shadow-lg">
        <h1 class="text-3xl font-bold mb-8 text-center text-[#f10707]">Carrito de Compras</h1>
        ...
        @else
            ...
            <div class="mt-6 font-semibold text-lg">
                ...
                <span class="text-[#f10707] text-xl">${{ number_format($total, 0) }}</span>
            </div>

            <form action="{{ route('comprador.cart.checkout') }}" method="POST" class="mt-4">
                @csrf
                <button type="submit" class="w-full bg-[#f10707] text-white py-2 px-4 rounded-md shadow hover:bg-blue-600 transition duration-200">Finalizar Compra</button>
            </form>
        @endif
    </div>
