...
<x-app-layout>
    <div class="container mx-auto mt-12 p-8 bg-white rounded-lg shadow-lg">
        ...
        @if (session('success'))
            <div id="success-message" class="bg-[#f10707] text-white p-4 rounded-lg mb-4">
                {{ session('success') }}
            </div>
        @endif
            ...
            @foreach($products as $product)
                <div class="border rounded-lg p-4">
                    ...
                    <form action="{{ route('comprador.cart.add', $product->id) }}" method="POST">
                        @csrf
                        <button type="submit" class="mt-2 bg-[#f10707] text-white rounded-lg py-2 px-4">Comprar</button>
                    </form>
                </div>
            @endforeach
        </div>
